...
#define DISPLAY_WIFI_MESSAGE  11
...
#define MESSAGE_WIFI_LENGTH 100
...
char messageWifi[MESSAGE_WIFI_LENGTH];
...
void loop()
{
  ...
  if (currentMillis - previousMillis >= interval)
  {
    ...
    currentAnimation = DISPLAY_WIFI_MESSAGE;
    ...
    case DISPLAY_WIFI_MESSAGE:
      ...
      if (message_incomplete)
      {
        ...
        if (font_ptr >= FONT_WIDTH)
        {
          ...
          Serial.print(" ");
          Serial.println(messageWifi[char_ptr]);
          if (messageWifi[char_ptr] == '\0')          
          {
            char_ptr = 0;
            message_incomplete = 0;
        ...
        {
          temp[temp_ptr] = font[messageWifi[char_ptr] - 32][font_ptr];
          font_ptr++;
        }
      }
...
void handleRoot()
{
  ...
  {
    //String temp = "ABC123";
    //temp.toCharArray(messageWifi, 10);
    char_ptr = 0;
    server.arg("NEWTEXT").toCharArray(messageWifi, MESSAGE_WIFI_LENGTH); //
...
{ // setup AP, start DNS server, start Web server
  const char *AP_NameChar = "The Hat";
  WiFi.mode(WIFI_AP);
